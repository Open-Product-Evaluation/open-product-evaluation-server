<template>
  <div class="barChart svg-container" :id="'barChart-' + this.id">
...
export default {
  ...
  methods: {
    draw() {
      setTimeout(() => {
        const drawData = this.data[this.id].barChart;
  ...
  mounted() {
    setTimeout(() => {
      ...
      const element = document.getElementById(`barChart-${this.id}`);
      this.lc = barChart.createBarChart(element);
